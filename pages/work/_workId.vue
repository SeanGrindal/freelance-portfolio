<template lang="html">
	<div class="Work">
		<div class="Work__content">
			...
			<work-row v-for="(row, i) in workPages[currentWorkPageNum].pageContent.rows" :key="`${i}`">
				<work-img v-if="row.type == 'img'" :imgData="row.imgData" :class="row.imgData.imgType" />

				<work-double-img v-else-if="row.type == 'doubleImg'" :imgs="row.imgs" />
				...
				<work-info
					...
					:heading="row.heading"
				>
					<work-img :imgData="row.img" :class="row.img.imgType" />
				</work-info>
			</work-row>
			...
			meta: [
				{
					hid: 'description',
					name: 'description',
					content: `Explore Sean Grindal's projects. ${
						this.workPages[this.currentWorkPageNum].description
					}.`
				}
			]
		}
